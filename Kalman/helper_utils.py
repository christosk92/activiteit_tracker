...
class KalmanPlotter:
    ...
    def plot_kalman_data(self, log):
        ...
        self.ax1.set_title("x-positie kalman filter")
        self.ax1.set_xlabel("tijd (s)")
        self.ax1.set_ylabel("x-positie (m)")
        self.ax1.plot(log.get_log('time'), log.get_log('x-positie'),
                 'o', label='x-positie', markersize=5)
        self.ax1.plot(log.get_log('time'), log.get_log('Benadering'),
                 '-', label='Benadering', markersize=5)
        self.ax1.plot(log.get_log('time'), log.get_log('Echt'),
                 '-', label='Echt', markersize=5)
        self.ax1.plot(log.get_log('time'), log.get_log('Bewegend gemiddeld'),
                 '-', label='Gemiddeld', markersize=5)
        self.ax1.legend(prop={'size': 10})
        # Plot the evolution of the state covariance
        self.ax2.set_xlabel("tijd (s)")
        self.ax2.set_ylabel("Staat Covariantie")
        self.ax2.plot(log.get_log('time'), log.get_log('Covariantie'),
                  '-', label='Staat Covariantie', markersize=5)
        plt.show()